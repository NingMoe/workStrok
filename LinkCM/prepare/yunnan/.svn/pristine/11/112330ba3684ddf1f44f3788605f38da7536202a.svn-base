<template>
    <div>
        <div class="total">
            ...
            <v-minbase v-if="item.minbase.status === true" :datas="mindatas"></v-minbase>
            <v-avoid v-if="item.avoid.status === true"></v-avoid>
            <div class="panel_0">
                <div class="lpanel">
                    <ul>
                        <li @click="showPanel($key)" v-for="it in item" :class="{'s-item':it.status}">
                            <em></em><label>{{it.name}}</label>
                        </li>
                    </ul>
                </div>
                <div class="count">
                    <ul><li v-for="it in datas">{{it.name}}<label>{{it.num}}</label></li></ul>
                </div>
            </div>
        </div>
...
<script>
    ...
    import Avoid from '../mainLegend/legendItems/Avoid';

    import { updateParam } from '../../../vuex/store'

    export default {
        props: ['datas', 'mindatas'],
        components: { 'v-target': Target, 'v-minbase': MinBase, 'v-quantown': QuanTown, 'v-avoid': Avoid },
        data() {
            return {
                item: {
                    target: { name: '靶向预警', status: false },
                    avoid: { name: '避灾导航', status: false },
                    townName: { name: '圈选镇名', status: false },
                    minbase: { name: '基础数据', status: true }
                },
                townTable: false,
...
.count { display: inline-block; width: 100%; height: auto; text-align: left; padding: 2px; border-top: 1px solid #ccc;
    ul li { width: auto; margin-right: 5px; height: 22px; line-height: 22px; float: left; min-width:98px;
...
.big .count { width: 100%; padding: 6px 5px;
    ul li { min-width: 140px; margin-right: 10px; }
}
.big .lpanel { height: 30px; line-height: 30px;
    ul { li { margin-right:50px;
            em { 
            	width: 22px; 
            	height: 22px; 
            	margin-top: -4px;

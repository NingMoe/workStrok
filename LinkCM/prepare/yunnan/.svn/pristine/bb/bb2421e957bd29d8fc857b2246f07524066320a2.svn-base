        ...
        <div class="target">
            ...
            <div class="msg_effect marTpad">
                ...
                <div class="effect-con">
                    ...
                    <ul>
                        <li v-for="it in town"><p title="{{ it }}">{{ it }}</p><em @click="delTown($key)">x</em></li>
                    </ul>
                </div>
            </div>
            <div class="msg_div marTpad">
    ...
    export default {
        ...
        vuex: {
            getters: {
                ...
                tempCacheId: state => state.model.tempCacheId,
        ...
        methods: {
            getAllTypes: function() {
                var url = this.dss+'/target/target!getDemagesType.action';
            ...
            getAllKinds: function() {
                ...
                var url = this.dss+'/target/target!getDemagesKindByType.action';
                var qdata = { typeCode: value };
            ...
            getAtwillCount: function() {
                let model = this.cType == '' ? this.pType : this.cType;
                let levels = this.levels + ',';
                var mTime = ModelTime.getModelTime(this.pType, this.cType, this.sourceTime, this.dateTime);
                model = levels + '#' + model + '#' + mTime.substring(0, 10);
                var url = this.dss+'/poi/poi!getPoiCount.action';
                var qdata = { areaCode: this.code, model: model };
            ...
            // 根据cache获取靶向发布人口、责任人等统计信息
            getAtwillByCache: function() {
                var url = this.dss+'/poi/poi!getPoiCountByCache.action';
                var qdata = {
                    areaCode: this.code,
                    cacheId: this.cacheId || this.tempCacheId
                };
                $.getJSON(url, qdata, (bd) => {
                    let count = this.count;
                    for (let key in count) {
            ...
            getAtwillTowns: function() {
                let url = this.dss+'/atwill/atwill!getAtwillTowns.action';
                let qdata = { cacheId: this.cacheId || this.tempCacheId, code: this.code };
                $.getJSON(url, qdata, (bd) => {
                    this.loadTown = false;
                    this.town = bd;
                });
            ...
            send: function() {
                ...
                if (this.warnMsg) {
                    ...
                    let url = this.dss+'/target/target!sendMsg.action';
                    let towns = this.town;
                    let effectCods = '';
                    for (let key in towns) {
                        effectCods += key + ','
        ...
        ready: function() {
            this.getAllTypes();
            if (!!this.ifcache) this.getAtwillByCache();
            else this.getAtwillCount();
            this.getAtwillTowns();
        },
        detached: function() {
            this.minStat = false;

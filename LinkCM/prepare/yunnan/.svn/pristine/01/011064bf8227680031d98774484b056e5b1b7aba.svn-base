...
window.FlexPaperViewerEmbedding = window.$f = function(id, args) {
    ...
    var _SWFFile, _PDFFile, _IMGFiles, _JSONFile = "",
        ...
        _cssDirectory = "",
        _localeDirectory = "";
    var _WMode = (config.WMode != null || config.wmmode != null ? config.wmmode || config.WMode : "window");
    ...
    _jsDirectory = (config.jsDirectory != null ? config.jsDirectory : "/dss-opp/resources/js");
...
(function() {
    ...
    FLEXPAPER.initLoginForm = function initLoginForm(IMGFiles, animate) {
        $(document.body).css('background-color', '#dedede');

        var img = new Image();
        $(img).bind('load', function() {
            $(document.body).append(
                "<div id='loginForm'>" +
                "<form class='flexpaper_htmldialog' method='POST' style='display:none;top:100px;margin:" + (($(window).height() > 500) ? "50px auto" : "0px auto") + "'>" +
            ...
            $('.flexpaper_htmldialog').css({
                height: '0px',
                display: 'block'
            });
            $('.flexpaper_htmldialog').animate({
                'height': theight + 'px',
                'top': '0px'
            }, {
                duration: anim_height_dur,
                complete: function() {
                    $('.flexpaper_htmldialog').css({
                        'height': ''
                    }); // remove height attribute to fit publication

                    $('.flexpaper_publication').animate({
                        opacity: 1
                    }, {
...
(function() {
    ...
    window.dispatchJQueryEvent = function(elementId, eventName, args) {
        $('#' + elementId).trigger(eventName, args);
    }
    ...
    function Flash(root, opts, conf) {
        ...
        if (conf.RenderingOrder.indexOf('html5') == 0) {
            if (confirm('The FlexPaper GPL version does not support HTML5 rendering. Do you want to navigate to our download page for more details?')) {
                location.href = 'http://flexpaper.devaldi.com/download.jsp?ref=FlexPaper'
            }
            return;
        }

        if (conf.RenderingOrder.indexOf('html') == 0) {
            if (confirm('The FlexPaper GPL version does not support HTML4 rendering. Do you want to navigate to our download page for more details?')) {
                location.href = 'http://flexpaper.devaldi.com/download.jsp?ref=FlexPaper'
            }
            return;
        }
    ...
    if (JQUERY) {
        $.fn.flashembed = function(opts, conf) {
            return this.each(function() {
                $(this).data("flashembed", flashembed(this, opts, conf));
            });
        };

        $.fn.FlexPaperViewer = function(args) {

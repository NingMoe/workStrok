            ...
            <div class="animate">
                ...
                <div v-show="isNormal">
                    <ul class="scale">
                        ...
                        <li v-for="item in hourItems" @mouseenter="hideMinBox(false, item)"  @mouseleave="hideMinBox(true)" @click="selectHour(item)" :class="{ 'select-time': item.isSelect }" :ldisabled="[item.isDisabled]">{{item.hour}}
                            <em :class="{ 'none': !item.isSelect, 'block': item.isSelect && isMin }"></em>
                        </li>
                ...
                <div class="time-scale-line">
                    <ul>
                        <li><em class="disabled"></em></li>
                        <li v-for="item in tagItems">
                            <em :class="{'disabled': item.isDisabled}"></em>
                        </li>
                        <li><em class="disabled"></em></li>
                    </ul>
                </div>
    ...
    export default {
        ...
        methods: {
            ...
            draw: function(range, isBaseAndSelect) {
                ...
                for (var e = endIndex; e < 8; e++) {
                
                    tempTagItems.push({
                        isDisabled: false
                    });
                    
                    tempHourItems.push(createTimeObj(tempStartTime, e, true));
                }
                ...
                if (isBaseAndSelect) {
                    if (!tempHourItems[7].isDisabled) {
                        tempObj = tempHourItems[7];
                ...
                this.selectTime(tempObj);
                this.baseTime = TimeUtil.convertToDate(tempHourItems[7].date);
                this.startTime = TimeUtil.convertToDate(tempHourItems[0].date);
                this.endTime = TimeUtil.convertToDate(tempHourItems[7].date);
